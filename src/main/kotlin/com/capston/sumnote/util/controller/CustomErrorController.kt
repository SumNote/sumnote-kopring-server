...
class CustomErrorController : ErrorController {
    ...
    fun handleError(request: HttpServletRequest): ResponseEntity<CustomApiResponse<Any?>> {
        ...
        // == 추가 ==
        val requestedUri = request.getAttribute(RequestDispatcher.ERROR_REQUEST_URI) as? String
        val message = requestedUri?.let { uri ->
            "요청 경로 '$uri'를 찾을 수 없습니다."
        } ?: "요청 경로를 찾을 수 없습니다."
        // == //
        ...
        status?.let {
            return when (it) {
                HttpStatus.NOT_FOUND.value() -> ResponseEntity
                    ...
                    .body(CustomApiResponse(HttpStatus.NOT_FOUND.value(), requestedUri, message))
